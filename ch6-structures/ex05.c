...
int main() {
  ...
  printf("\n===\nNow remove keys \"hello\" and \"something\".\n");
  undef("hello"); undef("something");

  printf("\n===\nAgain let's check my hashtab.\n");
  for (i = 0; i < HASHSIZE; i++) {
    np = hashtab[i];
    if (np != NULL)
      printf("%s: %s\n", np->name, np->defn);
...
void undef(char *s) {
  struct nlist *np, *npprev;

  for (np = hashtab[hash(s)]; np != NULL; npprev = np, np = np->next)
